        ...
        while (true) {
            ...
            if (display_service::IsInitialized()) {
                ...
                if (any_screen_refresh && !overlay_owns_screen) {
                    // When this batch also dismissed an overlay, the screen rebuild below
                    // supersedes the (otherwise dropped) overlay refresh. A page's own
                    // dismiss refresh is usually partial, which would leave the dismissed
                    // overlay's footprint as e-paper ghosting, so force a full-screen
                    // refresh. This is the global hook that makes every modal/overlay
                    // dismissal clear cleanly even when a page pairs it with its own refresh.
